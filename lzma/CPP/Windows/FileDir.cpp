      ...
      if (!RemoveDirectorySubItems2(pathPrefix, fileInfo))
		  return false;
								   ...
								   int &fileNamePartStartIndex)
			{
				...
				LPTSTR buffer = resultPath.GetBuf(MAX_PATH);
				DWORD needLength = ::GetFullPathName(fileName, MAX_PATH + 1,
													 buffer, &fileNamePointer);
				resultPath.ReleaseBuf_CalcLen(resultPath.Len());
...
#ifndef _UNICODE
			bool MyGetFullPathName(LPCWSTR fileName,
								   UString &resultPath,
								   int &fileNamePartStartIndex)
			{
			...
			static CSysString CSysConvertuint32_tToString(uint32_t value)
			{
				TCHAR buffer[32];
			...
			UINT CTempFile::Create(LPCTSTR dirPath, LPCTSTR prefix, CSysString &resultPath)
			{
				...
				resultPath += prefix;
				resultPath += '#';
				resultPath += CSysConvertuint32_tToString(number);
				resultPath += '@';
				resultPath += CSysConvertuint32_tToString(count);
				resultPath += ".tmp";
